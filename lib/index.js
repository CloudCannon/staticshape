...
const sourcePath = await p.text({
    ...
    validate(sourcePath) {
        ...
        if (path.normalize(sourcePath).includes('..'))
            return `Source path must be relative`;
...
const configPath = await p.text({
    ...
    validate(configPath) {
        ...
        if (!configPath.startsWith('./'))
            return `Config path must be relative`;
        if (path.normalize(configPath).includes('..'))
            return `Config path must be relative`;
...
const outputPath = await p.text({
    ...
    validate(outputPath) {
        ...
        if (!outputPath.startsWith('./'))
            return `Output path must be relative`;
        if (path.normalize(outputPath).includes('..'))
            return `Output path must be relative`;
