...
import { Element, Node } from 'angular-html-parser/lib/compiler/src/ml_parser/ast';
...
tests.forEach((def: TestDefinition) => {
    test(def.name, async (t: ExecutionContext) => {
        ...
        const tree = mergeChildren(0, primaryData, secondaryData, firstElement, secondElement);
